...
import { Fa500Px, FaMoon, FaSun } from 'react-icons/fa'
  ...
  return (
     ...
     <div className='fixed w-full bg-white dark:bg-blue-900 z-10 shadow-md'> 
       ...
       <div className='md:text-2xl 
        text-lg font-bold text-blue-600 dark:text-gray-100  cursor-pointer' >
        NewsUpdate
       </div>
       </Link>
       ...
       <div className='hidden md:flex space-x-6' >
          ...
          {
            links.map((lin)=>{
                return <Link to={`/${lin.toLowerCase() }`} key={lin} className='text-gray-700 dark:text-gray-200 dark:hover:text-white hover:text-blue-600 transition'>{lin} </Link>
                       })
          }
       </div>
